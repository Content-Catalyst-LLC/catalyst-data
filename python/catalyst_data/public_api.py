...
class CatalystApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/v1/catalog/status":
                self._json(200,DatasetCatalogService(self.server.repository).status()); return
            if path == "/v1/catalog/datasets":
                query=parse_qs(parsed.query); limit=min(500,max(1,int(query.get("limit",[50])[0]))); offset=max(0,int(query.get("offset",[0])[0]))
                items=DatasetCatalogService(self.server.repository).search(query=query.get("query",[None])[0],provider=query.get("provider",[None])[0],resource_kind=query.get("resource_kind",[None])[0],freshness=query.get("freshness",[None])[0],limit=limit,offset=offset)
                self._json(200,{"datasets":items,"pagination":{"limit":limit,"offset":offset,"returned":len(items)}}); return
            if path.startswith("/v1/catalog/datasets/"):
                catalog_id=unquote(path[len("/v1/catalog/datasets/"):]); item=DatasetCatalogService(self.server.repository).get(catalog_id)
                if item is None:
