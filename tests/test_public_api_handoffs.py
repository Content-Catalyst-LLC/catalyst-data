...
def test_typed_handoff_is_valid_and_receipt_is_immutable(tmp_path):
    repository = populated_repository(tmp_path)
    record = repository.list_records(limit=1)[0]
    handoff = create_handoff([record], target_product="decision-studio", target_capability="decision-evidence", source_version="1.12.0", api_base_url="https://data.example.org")
...
def test_handoff_rejects_unsupported_products(tmp_path):
    ...
    with pytest.raises(HandoffValidationError):
        create_handoff([record], target_product="unknown-product", target_capability="records", source_version="1.12.0")
...
def test_http_api_public_reads_protected_writes_openapi_and_handoffs(tmp_path):
    ...
    try:
        ...
        assert status == 200 and health["migration_version"] == 12
        ...
        handoff = create_handoff([records[0]], target_product="workbench", target_capability="calculation-input", source_version="1.12.0")
