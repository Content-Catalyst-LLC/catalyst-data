...
def test_migrations_are_contiguous_and_reversible(tmp_path):
    ...
    with connect(database) as connection:
        ...
        assert connection.execute("SELECT name FROM sqlite_master WHERE name='observation_batches'").fetchone() is not None
...
def test_repository_health_reports_current_schema(tmp_path):
    ...
    assert missing.latest_migration == 6
    repository.initialize()
    health = repository.health()
    assert health.healthy
    assert health.migration_version == 6
...
def test_migrations_three_through_five_rebuild_evidence_governance_and_lineage(tmp_path):
    ...
    assert repository.rollback(4) == [6, 5, 4, 3]
    ...
    assert repository.migrate() == [3, 4, 5, 6]
