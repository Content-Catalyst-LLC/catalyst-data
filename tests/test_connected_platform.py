...
def test_platform_schema_and_manifest_cover_all_v2_subsystems(tmp_path: Path) -> None:
    ...
    assert manifest["migration_version"] == 23
...
def test_integrated_platform_readiness_and_populated_rollback(tmp_path: Path) -> None:
    ...
    assert repository.rollback(1) == [23]
    assert repository.health().migration_version == 22
    assert repository.rollback(1) == [22]
    ...
    assert repository.rollback(1) == [16]
    ...
    assert repository.migrate() == [15, 16, 17, 18, 19, 20, 21, 22, 23]
